...
#define check_invariants(A)                                                 \
  check((A) != NULL, "array is NULL.");                                     \
  check((A)->contents != NULL, "array contents is NULL.");                  \
  check((A)->min_capacity <= (A)->capacity,                                 \
        "array capacity is smaller than min_capacity.");                    \
  check((A)->size <= (A)->capacity, "array size is larger than capacity."); \
  check((A)->element_size > 0, "element size is less than 0.");
...
static inline void* DArray_first(DArray* array) {
  check(array != NULL, "array is NULL.");
  check(array->contents != NULL, "array contents is NULL.");
...
static inline void* DArray_new(DArray* array) {
  void* el = NULL;
  check(array != NULL, "array is NULL.");
  check(array->element_size > 0, "element size is less than 0.");
